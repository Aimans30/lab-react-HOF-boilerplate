...
class HigherOrderComponent extends Component {
    ...
    renderItemsByAge = (minAge, maxAge) => {
        const data = this.state.userData;
        const filteredData = data.filter(item => item.age > minAge && item.age <= maxAge);
        const mapRows = filteredData.map((item) => (
            <React.Fragment key={item.id}>
                <li className="list-elements">
                    <span>Id: {item.id}</span>
    ...
    render() {
        return (
            <React.Fragment>
                ...
                <h1>Display items based on UserType</h1>
                <div className="display-box">
                ...
                <div className="display-box">
                    <ul>{this.renderItemsByAge(28, 50)}</ul>
                </div>
                <h1>Total Experience of Designers: {this.findTotalExperienceOfDesigners()} years</h1>
            </React.Fragment>
        );
    }
}
